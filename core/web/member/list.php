...
class List_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		foreach ($list as $val) {
			...
			$list_jiedianren[] = trim($val['jiedianrenid'],',');
		}
		...
		if (!empty($list_jiedianren)) {
			$res_jiedcianren = pdo_fetchall('select id,nickname as jiedianrennickname,avatar as jiedianrenavatar from ' . tablename('ewei_shop_member') . 'where id in(' . implode(',', $list_jiedianren) . ')',array(), 'id');
		}
		...
		foreach ($list as &$row) {
			...
			$row['jiedianrennickname'] = isset($res_jiedianren[$row['jiedianrenid']])? $res_jiedianren[$row['jiedianrenid']]['jiedianrennickname']:'';
			$row['agentavatar'] = isset($res_agent[$row['agentid']]) ? $res_agent[$row['agentid']]['agentavatar'] : '';
			$row['jiedianrenavatar'] = isset($res_jiedianren[$row['jiedianrenid']]) ? $res_jiedianren[$row['jiedianrenid']]['jiedianrenavatar']:'';
	...
	public function detail()
	{
		...
		if (!empty($member['jiedianrenid'])) {
			$parentjiedianren = m('member')->getMember($member['jiedianrenid']);
		}
			...
			if ($hascommission) {
				if (cv('commission.agent.edit')) {
					...
					if (!empty($adata)) {
						...
						if ($adata['jiedianrenid'] != $member['jiedianrenid']) {
							if (cv('commission.agent.changeagent')) {
								plog('commission.agent.changeagent', '修改jiedianren <br/> 会员信息:  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile'] . ' <br/>上级ID: ' . $member['jiedianrenid'] . ' -> 新上级ID: ' . $adata['jiedianrenid'] . '; <br/> 固定上级: ' . ($member['fixagentid'] ? '是' : '否') . ' -> ' . ($adata['fixagentid'] ? '是' : '否'));
							}
							else {
								$adata['jiedianrenid'] = $member['jiedianrenid'];
							}
						}
				...
				if (cv('globonus.partner.check')) {
					...
					if (!empty($gdata)) {
						if (empty($_GPC['oldpartnerstatus']) && $gdata['partnerstatus'] == 1) {
							...
							plog('globonus.partner.check', '审核会员 <br/>会员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
						}

						plog('globonus.partner.edit', '修改会员 <br/>会员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
