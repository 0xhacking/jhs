...
class Upgrade_EweiShopV2Page extends SystemPage
{
	...
	public function check()
	{
		...
		set_time_limit(0);
		$auth = get_auth();
		if(!$auth['code'])show_json(0, '分销商城可以在线升级 <a href="index.php?c=cloud&a=52jscn_renren" target=“_blank”>点击立即查看最新版本</a>');
		...
		$resp = ihttp_request('http://cloud.52jscn.com/gateway.php/a=rr&b=check', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', trim($_W['siteroot'], '/'))), 'version' => $version, 'release' => $release, 'manual' => 1, 'plugins' => array_keys($plugins)));
		$templatefiles = '';
	...
	public function process()
	{
		...
		if ($action == 'file') {
			...
			if (!empty($path)) {
				$resp = ihttp_request('http://cloud.52jscn.com/gateway.php/a=rr&b=download', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'path' => $path));
