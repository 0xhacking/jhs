...
//		if (empty($leveltype)) {
			//$ordermoney = pdo_fetch('select sum(og.realprice) as price from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid ' . ' where o.openid=:openid and o.status>0 and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
		   //$ordermoney = pdo_fetch('select sum(price) from ' . tablename('ewei_shop_order') . (' where ' . $orderid . '  = id  '), array(':orderid' => $orderid));
		   //show_json(0,$orderid.'-'.$ordermoney['price']);
			//$level = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . (' where uniacid=:uniacid  and enabled=1 and ' . $ordermoney['price'] . ' >= ordermoney and ordermoney>0  order by level desc limit 1'), array(':uniacid' => $_W['uniacid']));
        	//show_json(1,$member['openid']);
			//show_json(1,$level);
		//show_json(1,$ordermoney);
			//		}
//		else {
//			if ($leveltype == 1) {
...
//			}
//		}
			//show_json(1,$ordermoney['price']);
        //show_json(0,$level['id']);
        //此处查询并非是查询订单总金额，而是查询定订单中套餐区商品价格之和
        //获得最新订单
        $new_order = pdo_fetch('select * from '.tablename('ewei_shop_order').' where openid=:openid  and status>0 and uniacid=:uniacid order by id desc limit 1 ', array(':uniacid' => $_W[uniacid], ':openid' => $member[openid]));
            //print_r($new_order);
        //$ordermoney = pdo_fetch('select sum(og.realprice) as price from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid ' . ' where o.openid=:openid and o.status>0 and o.uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
        $ordermoney = pdo_fetch('select sum(og.realprice) as price from ' . tablename('ewei_shop_order_goods') . ' og  left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid  left join '. tablename('ewei_shop_goods').'g on g.id = og.goodsid  where o.openid=:openid and g.pcate = "1176" and o.status>0 and o.uniacid=:uniacid and o.id = "'.$new_order['id'].'"' , array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
        $level = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . (' where uniacid=:uniacid  and enabled=1 and ' . $ordermoney['price'] . ' >= ordermoney and ordermoney>0  order by level desc limit 1'), array(':uniacid' => $_W['uniacid']));
        //show_json(1,$ordermoney);
		...
		if ($canupgrade) {
            ...
            pdo_update('ewei_shop_member', array('agentlevel' => $level['id']), array('id' => $member['id']));
            $pluginc = p("commission");
            if( $pluginc && $orderid!=0 )
            {
                $pluginc->calculate($orderid, true);
            }
			com_run('wxcard::updateMemberCardByOpenid', $openid);
