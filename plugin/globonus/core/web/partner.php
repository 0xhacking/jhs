...
class Partner_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		if ($_GPC['export'] == '1') {
			...
			plog('globonus.partner.export', '导出会员数据');
	...
	'columns' => array(
		...
		array('title' => '成为会员时间', 'field' => 'partneragenttime', 'width' => 12),
	...
	public function delete()
	{
		...
		foreach ($members as $member) {
			...
			plog('globonus.partner.delete', '取消会员资格 <br/>会员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		}
	...
	public function check()
	{
		...
		foreach ($members as $member) {
			...
			if ($status == 1) {
				...
				plog('globonus.partner.check', '取消审核 <br/>会员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
	...
	public function partnerblack()
	{
		...
		foreach ($members as $member) {
			...
			if ($partnerblack == 1) {
				...
				plog('globonus.partner.partnerblack', '设置黑名单 <br/>会员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else {
				pdo_update('ewei_shop_member', array('ispartner' => 1, 'partnerstatus' => 1, 'partnerblack' => 0), array('id' => $member['id']));
				plog('globonus.partner.partnerblack', '取消黑名单 <br/>会员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
