...
class Bonus_EweiShopV2Page extends PluginWebPage
{
	...
	public function detail()
	{
		...
		if ($_GPC['export'] == 1) {
			...
			plog('globonus.bonus.detail.export', '导出结算单会员数据 ID: ' . $data['id'] . ' 单号: ' . $data['billno']);
	...
	public function delete()
	{
		...
		plog('globonus.bonus.delete', '删除结算单 ID:' . $data . ' 单号: ' . $data['billno'] . ' 分红金额: ' . $data['bonusmoney_pay'] . ' 会员人数:  ' . $data['partnercount']);
		show_json(1);
	}
	...
	public function build()
	{
		...
		if ($_W['ispost']) {
			...
			plog('globonus.bonus.build', '创建结算单 ID:' . $billid . ' 单号: ' . $bill['billno'] . ' 分红金额: ' . $bill['bonusmoney_pay'] . ' 会员人数:  ' . $bill['partnercount']);
	...
	public function payp()
	{
		...
		if (!empty($member)) {
			$result = m('finance')->pay($partner['openid'], $moneytype, $pay, $partner['payno'], '周分红', false);
