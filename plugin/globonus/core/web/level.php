...
class Level_EweiShopV2Page extends PluginWebPage
{
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				if ($id == 'default') {
					...
					$this->updateSet($set);
					plog('globonus.level.edit', '修改会员默认等级' . $updatecontent);
				}
				else {
					...
					plog('globonus.level.edit', '修改会员等级 ID: ' . $id . $updatecontent);
				}
			}
			else {
				pdo_insert('ewei_shop_globonus_level', $data);
				$id = pdo_insertid();
				plog('globonus.level.add', '添加会员等级 ID: ' . $id);
			}
	...
	public function delete()
	{
		...
		foreach ($items as $item) {
			...
			plog('globonus.level.delete', '删除会员等级 ID: ' . $id . ' 等级名称: ' . $item['levelname']);
		}
