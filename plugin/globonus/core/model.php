				...
				if ($message_type == TM_GLOBONUS_UPGRADE && empty($usernotice['globonus_upgrade'])) {
					if ($tm['is_advanced']) {
						...
						$message = array(
							...
							'keyword2' => array('title' => '业务状态', 'value' => '会员等级升级通知', 'color' => '#000000'),
							'keyword3' => array('title' => '业务内容', 'value' => '您的会员等级从' . $data['oldlevel']['levelname'] . '升级到' . $data['newlevel']['levelname'] . ',特此通知！', 'color' => '#000000'),
							'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
							'remark'   => array('value' => "\n感谢您的支持", 'color' => '#000000')
							);
						$toopenid = $openid;
						...
						$msg = array(
							'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
							'keyword2' => array('value' => !empty($tm['upgradetitle']) ? $tm['upgradetitle'] : '会员等级升级通知', 'color' => '#73a68d'),
							'keyword3' => array('value' => $message, 'color' => '#73a68d')
							);
					...
					if ($message_type == TM_GLOBONUS_BECOME && empty($usernotice['globonus_become'])) {
						if ($tm['is_advanced']) {
							...
							$tag = 'globonus_become';
							$text = '恭喜您成为' . $set['texts']['center'] . "的会员！\n" . date('Y-m-d H:i') . "\n";
							$message = array(
								'first'    => array('value' => '亲爱的' . $member['nickname'] . '，恭喜您成为' . $set['texts']['center'] . '的会员', 'color' => '#ff0000'),
								'keyword2' => array('title' => '业务状态', 'value' => '成为会员通知', 'color' => '#000000'),
								'keyword3' => array('title' => '业务内容', 'value' => '恭喜您成为会员', 'color' => '#000000'),
								'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
								'remark'   => array('value' => "\n感谢您的支持", 'color' => '#000000')
								);
							$toopenid = $openid;
							...
							$msg = array(
								'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
								'keyword2' => array('value' => !empty($tm['becometitle']) ? $tm['becometitle'] : '成为会员通知', 'color' => '#73a68d'),
								'keyword3' => array('value' => $message, 'color' => '#73a68d')
								);
		...
		public function upgradeLevelByOrder($openid)
		{
			...
			if ($leveltype == 4 || $leveltype == 5) {
				...
				if (empty($oldlevel['id'])) {
					$oldlevel = array('levelname' => empty($set['levelname']) ? '普通会员' : $set['levelname'], 'bonus' => $set['bonus']);
				}
