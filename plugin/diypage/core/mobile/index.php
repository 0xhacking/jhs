		...
		if( $urlData[0] == "goodsids" || $urlData[0] == "category" || $urlData[0] == "groups" ) 
		{
			...
			else 
			{
				if( $urlData[0] == "groups" ) 
				{
					...
					$goodsql = "SELECT id,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,isdiscount,hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,maxprice,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM " . tablename("ewei_shop_goods") . " WHERE id in(" . $goodsid . ") AND uniacid =" . $_W["uniacid"] . " limit 0," . $goodsnum;
