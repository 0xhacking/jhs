...
class Apply_EweiShopV2Page extends PluginWebPage
{
			...
			foreach ($list as &$row) {
				...
				foreach ($orders as &$order) {
					...
					foreach ($goods as $gindex => &$g) {
						...
						//v1.000 start
						$commissionleadmoney = iunserializer($g['commissionlead']);
						if ($order['level']==4) {
							if ($g['statuslead1'] == 3) {
								$totalpay += $commissionleadmoney['level1'];
								$g['cstatus'] = '已打款';
							}
							else if ($g['statuslead1'] == 2) {
								$passmoney += $commissionleadmoney['level1'];
								$g['cstatus'] = '待打款';
							}
							else if ($g['statuslead1'] == 0) {
								$g['cstatus'] = '未审核';
							}
							else if ($g['statuslead1'] == -1) {
								$g['cstatus'] = '未通过';
							}
						}
						if ($order['level']==5) {
							if ($g['statuslead2'] == 3) {
								$totalpay += $commissionleadmoney['level2'];
								$g['cstatus'] = '已打款';
							}
							else if ($g['statuslead2'] == 2) {
								$passmoney += $commissionleadmoney['level2'];
								$g['cstatus'] = '待打款';
							}
							else if ($g['statuslead2'] == 0) {
								$g['cstatus'] = '未审核';
							}
							else if ($g['statuslead2'] == -1) {
								$g['cstatus'] = '未通过';
							}
						}
						if ($order['level']==6) {
							if ($g['statuslead3'] == 3) {
								$totalpay += $commissionleadmoney['level3'];
								$g['cstatus'] = '已打款';
							}
							else if ($g['statuslead3'] == 2) {
								$passmoney += $commissionleadmoney['level3'];
								$g['cstatus'] = '待打款';
							}
							else if ($g['statuslead3'] == 0) {
								$g['cstatus'] = '未审核';
							}
							else if ($g['statuslead3'] == -1) {
								$g['cstatus'] = '未通过';
							}
						}
						if ($order['level']==7) {
							if ($g['statuslead4'] == 3) {
								$totalpay += $commissionleadmoney['level4'];
								$g['cstatus'] = '已打款';
							}
							else if ($g['statuslead4'] == 2) {
								$passmoney += $commissionleadmoney['level4'];
								$g['cstatus'] = '待打款';
							}
							else if ($g['statuslead4'] == 0) {
								$g['cstatus'] = '未审核';
							}
							else if ($g['statuslead4'] == -1) {
								$g['cstatus'] = '未通过';
							}
						}
						if ($order['level']==8) {
							if ($g['statuslead5'] == 3) {
								$totalpay += $commissionleadmoney['level5'];
								$g['cstatus'] = '已打款';
							}
							else if ($g['statuslead5'] == 2) {
								$passmoney += $commissionleadmoney['level5'];
								$g['cstatus'] = '待打款';
							}
							else if ($g['statuslead5'] == 0) {
								$g['cstatus'] = '未审核';
							}
							else if ($g['statuslead5'] == -1) {
								$g['cstatus'] = '未通过';
							}
						}
						//v1.000 end
	...
	protected function applyData()
	{
		...
		$agentid = $apply['mid'];
		$member = $this->model->getInfo($agentid, array('total', 'ok', 'apply', 'lock', 'check', 'wait', 'pay'));
		...
		foreach ($list as &$row) {
			...
			//v1.000  start 修改
			$goods = pdo_fetchall('select og.id,g.thumb,og.price,og.realprice, og.total,g.title,o.paytype,og.optionname,og.commission1,og.commission2,og.commission3,og.commissions,og.commissionlead,og.statuslead1,og.statuslead2,og.statuslead3,og.statuslead4,og.statuslead5,og.mmoney,og.status1,og.status2,og.status3,og.content1,og.content2,og.content3 from ' . tablename('ewei_shop_order_goods') . ' og' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id=og.goodsid  ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid  ' . ' where og.uniacid = :uniacid and og.orderid=:orderid and og.nocommission=0 order by og.createtime  desc ', array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
			//v1.000 end
			foreach ($goods as &$g) {
				//v1.000 start
				$commissions = iunserializer($g['commissions']);
				//v1.000 end
				$commissionleadstr = iunserializer($g['commissionlead']);
				if (1 <= $this->set['level']) {
					$commission = iunserializer($g['commission1']);
				...
				//v1.000 start
				if($row['level'] ==4 ){
					$totalcommission += $commissionleadstr['level1'];
					$g['commissionlead1'] = $commissionleadstr['level1'];
					if (2 <= $g['statuslead1']) {
						$totalpay += $commissionleadstr['level1'];
					}
				}
				if($row['level'] ==5 ){
					$totalcommission += $commissionleadstr['level2'];
					$g['commissionlead2'] = $commissionleadstr['level2'];
					if (2 <= $g['statuslead2']) {
						$totalpay += $commissionleadstr['level2'];
					}
				}
				if($row['level'] ==6 ){
					$totalcommission += $commissionleadstr['level3'];
					$g['commissionlead3'] = $commissionleadstr['level3'];
					if (2 <= $g['statuslead3']) {
						$totalpay += $commissionleadstr['level3'];
					}
				}
				if($row['level'] ==7 ){
					$totalcommission += $commissionleadstr['level4'];
					$g['commissionlead4'] = $commissionleadstr['level4'];
					if (2 <= $g['statuslead4']) {
						$totalpay += $commissionleadstr['level4'];
					}
				}
				if($row['level'] ==8 ){
					$totalcommission += $commissionleadstr['level5'];
					$g['commissionlead5'] = $commissionleadstr['level5'];
					if (2 <= $g['statuslead5']) {
						$totalpay += $commissionleadstr['level5'];
					}
				}
				//v1.000 end
		...
		// v1.003 start
			$totalcommission += ($member["monthmoney_zhuangtai"]==0 ? 0 : $member["monthmoney_old_no"]);
			$totalpay += ($member["monthmoney_zhuangtai"]==0 ? 0 : $member["monthmoney_old_no"]);
		// v1.003 end
	...
	public function check()
	{
		...
		foreach ($ogids as $ogid) {
			//v1.000 start 修改
			$g = pdo_fetch('SELECT total, commission1,commission2,commission3,commissions,commissionlead from ' . tablename('ewei_shop_order_goods') . '  ' . 'where id=:id and uniacid = :uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $ogid));
			//v1.000 end
            // v1.009 start
            //$mmber = pdo_fetch('SELECT monthmoney_zhuangtai from' . tablename('ewei_shop_member')  . ' where id=:id and uniacid = :uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $ogid));
            //详细查询
            $mmber = pdo_fetch(" select monthmoney_zhuangtai from  " . tablename('ims_ewei_shop_member') ." a left join  " .tablename('ims_ewei_shop_order')." b on a.id = b.agentid left join ".tablename('ims_ewei_shop_order_goods') ." c on b.id = c.orderid  where c.id = " .$ogid ."limit 1");
            // v1.009 end
			if (empty($g)) {
				continue;
			}
			...
			//v1.000 start
			else if (isset($_GPC['statuslead1'][$ogid])) {
				if (intval($_GPC['statuslead1'][$ogid]) == 2) {
					$paycommission += $commissionlead['level1'];
					$isAllUncheck = false;
				}

				$update = array('statuslead1' => intval($_GPC['statuslead1'][$ogid]));

			}
			else if (isset($_GPC['statuslead2'][$ogid])) {
				if (intval($_GPC['statuslead2'][$ogid]) == 2) {
					$paycommission += $commissionlead['level2'];
					$isAllUncheck = false;
				}

				$update = array('statuslead2' => intval($_GPC['statuslead2'][$ogid]));

			}
			else if (isset($_GPC['statuslead3'][$ogid])) {
				if (intval($_GPC['statuslead3'][$ogid]) == 2) {
					$paycommission += $commissionlead['level3'];
					$isAllUncheck = false;
				}

				$update = array('statuslead3' => intval($_GPC['statuslead3'][$ogid]));

			}
			else if (isset($_GPC['statuslead4'][$ogid])) {
				if (intval($_GPC['statuslead4'][$ogid]) == 2) {
					$paycommission += $commissionlead['level4'];
					$isAllUncheck = false;
				}

				$update = array('statuslead4' => intval($_GPC['statuslead4'][$ogid]));

			}else {
                if (isset($_GPC['statuslead5'][$ogid])) {
                    if (intval($_GPC['statuslead5'][$ogid]) == 2) {
                        $paycommission += $commissionlead['level5'];
                        $isAllUncheck = false;
                    }

                    $update = array('statuslead5' => intval($_GPC['statuslead5'][$ogid]));
                }
            }
		...
		// v1.009 start 判断提交的时候月分红是否已经提现，如提现则只显示其他商品提现申请，如未体现则一并显示，这里主要作用是当月分红单独体现时，生成体现申请
		...
		if ($isAllUncheck) {
            ...
            // v1.009 start
            if($member['monthmoney_zhuangtai'] != 3){
                pdo_update('ewei_shop_member',array('monthmoney_zhuangtai' => 0),array('id'=>$apply['mid']));
            }
            // v1.009 end
		}
		else {
			...
			// v1.003 start
                // v1.009 start 判断本月是否已经体现过月分红
                    if($member['monthmoney_zhuangtai']!=3){
                        pdo_update('ewei_shop_member',array('monthmoney_zhuangtai' => 2),array('id'=>$apply['mid']));
                    }
	...
	public function cancel()
	{
		...
		foreach ($list as $row) {
			...
			foreach ($row['goods'] as $g) {
				...
				//v1.000 start
					...
					$update = array('statuslead1' => 1);
				}
				else if ($row['level'] == 5) {
					$update = array('statuslead2' => 1);
				}
				else if ($row['level'] == 6) {
					$update = array('statuslead3' => 1);
				}
				else if ($row['level'] == 7) {
					$update = array('statuslead4' => 1);
				}
				else if ($row['level'] == 8) {
					$update = array('statuslead5' => 1);
				}
				//v1.000 end
				if (!empty($update)) {
		...
		// v1.003 start
            //v1.009 start
                if($member['monthmoney_zhuangtai'] != 3){
                    pdo_update('ewei_shop_member',array('monthmoney_zhuangtai' => 1),array('id'=>$apply['mid']));
                }
            //v1.009 end
		// v1.003 end
		...
		foreach ($list as $row) {
			...
			foreach ($row['goods'] as $g) {
				...
				//v1.000 start
					...
					$update = array('statuslead1' => 0);
				}
				else if ($row['level'] == 5){
					$update = array('statuslead2' => 0);
				}
				else if ($row['level'] == 6){
					$update = array('statuslead3' => 0);
				}
				else if ($row['level'] == 7){
					$update = array('statuslead4' => 0);
				}
				else if ($row['level'] == 8){
					$update = array('statuslead5' => 0);
				}
				//v1.000 end
	...
	public function pay($params = array(), $mine = array())
	{
		...
		foreach ($list as $row) {
			...
			foreach ($row['goods'] as $g) {
				...
				//v1.000 end
				//v1.000 start
				else if ($row['level'] == 4 && $g['statuslead1'] == 2) {
					$update = array('statuslead1' => 3);
				}
				else if ($row['level'] == 5 && $g['statuslead2'] == 2) {
					$update = array('statuslead2' => 3);
				}
				else if ($row['level'] == 6 && $g['statuslead3'] == 2) {
					$update = array('statuslead3' => 3);
				}
				else if ($row['level'] == 7 && $g['statuslead4'] == 2) {
					$update = array('statuslead4' => 3);
				}
				else if ($row['level'] == 8 && $g['statuslead5'] == 2) {
					$update = array('statuslead5' => 3);
				}
				//v1.000 end
				if (!empty($update)) {
					pdo_update('ewei_shop_order_goods', $update, array('id' => $g['id']));
		...
		// v1.003 start
			//v1.009 start
			pdo_update('ewei_shop_member',array('monthmoney_zhuangtai' => 3, 'monthmoney_ok_time' => time(), 'monthmoney_ok'=>$member['commission_old_no'], 'monthmoney_old' => 0, 'monthmoney_old_no' => 0),array('id'=>$apply['mid']));
			//v1.009 end
        // v1.003 end
        // v1.015 start 解决月份红发放、提现记录问题
        	pdo_update('ewei_shop_monthmoney',array('is_ti' => 1,),array('uid'=>$apply['mid']));
        // v1.015 end
		show_json(1, array('url' => webUrl('commission/apply', array('status' => $apply['status']))));
	}
	...
	public function payed($params = array(), $mine = array())
	{
		...
		// v1.003 start
		//pdo_update('ewei_shop_member',array('monthmoney_zhuangtai' => 3, 'monthmoney_ok_time' => time(), 'monthmoney_ok' => $mcommission, 'monthmoney_old_ok' => 0),array('id'=>$apply['mid']));
		// v1.003 end
		show_json(1, array('url' => webUrl('commission/apply', array('status' => $apply['status']))));
	}
