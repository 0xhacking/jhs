...
class Agent_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.id as pid,p.nickname as parentname,t.nickname as parentjiedianrenname,t.avatar as parentjiedianrenavatar,p.avatar as parentavatar,f.follow as followed, f.unfollowtime from ' . tablename('ewei_shop_member') . ' dm ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join '. tablename('ewei_shop_member') . ' t on t.id = dm.jiedianrenid ' .  ' left join ' . tablename('ewei_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('mc_mapping_fans') . ('f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid']) . ' where dm.uniacid = ' . $_W['uniacid'] . (' and dm.isagent =1  ' . $condition . ' ORDER BY dm.agenttime desc');
	...
	public function query_jiedianren()
	{
		global $_W;
		global $_GPC;
		$kwd = trim($_GPC['keyword']);
		...
		include $this->template('commission/query_jiedianren');
	}
