...
if( !class_exists("CommissionModel") ) 
{
	class CommissionModel extends PluginModel 
	{
		...
		public function calculate($orderid = 0, $update = true, $order_agentid = NULL) 
		{
			...
			$goods = pdo_fetchall("select og.id,og.realprice,og.total,g.hasoption,g.marketprice,og.goodsid,og.optionid,g.hascommission,g.nocommission,g.pcate,g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from " . tablename("ewei_shop_order_goods") . "  og " . " left join " . tablename("ewei_shop_goods") . " g on g.id = og.goodsid" . " where og.orderid=:orderid and og.uniacid=:uniacid", array( ":orderid" => $orderid, ":uniacid" => $_W["uniacid"] ));


			// $goods = pdo_fetchall("select og.id,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,g.hascommission,g.nocommission, g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,g.commission3_rate,g.commission3_pay,g.commission,og.commissions,og.seckill,og.seckill_taskid,og.seckill_timeid from " . tablename("ewei_shop_order_goods") . "  og " . " left join " . tablename("ewei_shop_goods") . " g on g.id = og.goodsid" . " where og.orderid=:orderid and og.uniacid=:uniacid", array( ":orderid" => $orderid, ":uniacid" => $_W["uniacid"] ));
			...
			if( 0 < $set["level"] ) 
			{
				foreach( $goods as &$cinfo ) 
				{
					...
					if( $update ) 
					{
						...
						$commissionlead =  array("level1" => 0,"level2"=>0,"level3"=> 0,"level4"=>0,"level5"=>0); 
...
//											$money = $l1["ping1money"];
//										}
										if ($cinfo["realprice"]>$l1["ping2down"] && $cinfo["realprice"]<=$l1["ping2up"]) {
											$money = $l1["ping2money"];
										}
										if ($cinfo["realprice"]>$l1["ping3down"] && $cinfo["realprice"]<=$l1["ping3up"]) {
											$money = $l1["ping3money"];
										}
										if ($cinfo["realprice"]>$l1["ping4down"] && $cinfo["realprice"]<=$l1["ping4up"]) {
											$money = $l1["ping4money"];
										}
										$commissions["level1"]=$money;
										if (!empty($m1["agentid"])) {
											$m2 = m("member")->getMember($m1["agentid"]);
											$l2 = $this->getLevel($m2["openid"]);
											if ($cinfo["realprice"]>$l2["ping1down"] && $cinfo["realprice"]<=$l2["ping1up"]) {
												$money = $l2["ping1money2"];
											}
											if ($cinfo["realprice"]>$l2["ping2down"] && $cinfo["realprice"]<=$l2["ping2up"]) {
												$money = $l2["ping2money2"];
											}
											if ($cinfo["realprice"]>$l2["ping3down"] && $cinfo["realprice"]<=$l2["ping3up"]) {
												$money = $l2["ping3money2"];
											}
											if ($cinfo["realprice"]>$l2["ping4down"] && $cinfo["realprice"]<=$l2["ping4up"]) {
												$money = $l2["ping4money2"];
											}
											$commissions["level2"]=$money;
											if (!empty($m2["agentid"])) {
												$m3 = m("member")->getMember($m2["agentid"]);
												$l3 = $this->getLevel($m3["openid"]);
												if ($cinfo["realprice"]>$l3["ping1down"] && $cinfo["realprice"]<=$l3["ping1up"]) {
													$money = $l3["ping1money3"];
												}
												if ($cinfo["realprice"]>$l3["ping2down"] && $cinfo["realprice"]<=$l3["ping2up"]) {
													$money = $l3["ping2money3"];
												}
												if ($cinfo["realprice"]>$l3["ping3down"] && $cinfo["realprice"]<=$l3["ping3up"]) {
													$money = $l3["ping3money3"];
												}
												if ($cinfo["realprice"]>$l3["ping4down"] && $cinfo["realprice"]<=$l3["ping4up"]) {
													$money = $l3["ping4money3"];
												}
												$commissions["level3"]=$money;
											}
										}
									} else {
					 				...
					 				if(!empty($m1["agentid"])){
					 					...
					 					if(!empty($m2["agentid"])){
					 						...
					 						if(!empty($m3["agentid"])){
					 							...
					 							$l4 = $this->getLevel($m4["openid"]);
					 							if ($l4["levelname"]=="消费股东" or $l4["levelname"]=="公司合伙人") {
					 								if ($cinfo["realprice"]>$l4["ling1down"] && $cinfo["realprice"]<=$l4["ling1up"]) {
														$money = $l4["ling1money"];
													}
													if ($cinfo["realprice"]>$l4["ling2down"] && $cinfo["realprice"]<=$l4["ling2up"]) {
														$money = $l4["ling2money"];
													}
													if ($cinfo["realprice"]>$l4["ling3down"] && $cinfo["realprice"]<=$l4["ling3up"]) {
														$money = $l4["ling3money"];
													}
													if ($cinfo["realprice"]>$l4["ling4down"] && $cinfo["realprice"]<=$l4["ling4up"]) {
														$money = $l4["ling4money"];
													}
													$commissionlead["level1"]= $money;
												}
												if (!empty($m4["agentid"])) {
													$m5 = m("member")->getMember($m4["agentid"]);
					 								$l5 = $this->getLevel($m5["openid"]);
					 								if ($l5["levelname"]=="消费股东" or $l5["levelname"]=="公司合伙人") {
					 									if ($cinfo["realprice"]>$l5["ling1down"] && $cinfo["realprice"]<=$l5["ling1up"]) {
															$money = $l5["ling1money"];
														}
														if ($cinfo["realprice"]>$l5["ling2down"] && $cinfo["realprice"]<=$l5["ling2up"]) {
															$money = $l5["ling2money"];
														}	
														if ($cinfo["realprice"]>$l5["ling3down"] && $cinfo["realprice"]<=$l5["ling3up"]) {
															$money = $l5["ling3money"];
														}
														if ($cinfo["realprice"]>$l5["ling4down"] && $cinfo["realprice"]<=$l5["ling4up"]) {
															$money = $l5["ling4money"];
														}
														$commissionlead["level2"]= $money;
													}
													if (!empty($m5["agentid"])) {
														$m6 = m("member")->getMember($m5["agentid"]);
					 									$l6 = $this->getLevel($m6["openid"]);
					 									if ($l6["levelname"]=="消费股东" or $l6["levelname"]=="公司合伙人") {
					 										if ($cinfo["realprice"]>$l6["ling1down"] && $cinfo["realprice"]<=$l6["ling1up"]) {
																$money = $l6["ling1money"];
															}
															if ($cinfo["realprice"]>$l6["ling2down"] && $cinfo["realprice"]<=$l6["ling2up"]) {
																$money = $l6["ling2money"];
															}
															if ($cinfo["realprice"]>$l6["ling3down"] && $cinfo["realprice"]<=$l6["ling3up"]) {
																$money = $l6["ling3money"];
															}
															if ($cinfo["realprice"]>$l6["ling4down"] && $cinfo["realprice"]<=$l6["ling4up"]) {
																$money = $l6["ling4money"];
															}
															$commissionlead["level3"]= $money;
														}
														if (!empty($m6["agentid"])) {
															$m7 = m("member")->getMember($m6["agentid"]);
					 										$l7 = $this->getLevel($m7["openid"]);
					 										if ($l7["levelname"]=="消费股东" or $l7["levelname"]=="公司合伙人") {
					 											if ($cinfo["realprice"]>$l7["ling1down"] && $cinfo["realprice"]<=$l7["ling1up"]) {
																	$money = $l7["ling1money"];
																}
																if ($cinfo["realprice"]>$l7["ling2down"] && $cinfo["realprice"]<=$l7["ling2up"]) {
																	$money = $l7["ling2money"];
																}
																if ($cinfo["realprice"]>$l7["ling3down"] && $cinfo["realprice"]<=$l7["ling3up"]) {
																	$money = $l7["ling3money"];
																}
																if ($cinfo["realprice"]>$l7["ling4down"] && $cinfo["realprice"]<=$l7["ling4up"]) {
																	$money = $l7["ling4money"];
																}
																$commissionlead["level4"]= $money;
															}
															if (!empty($m7["agentid"])) {
																$m8 = m("member")->getMember($m7["agentid"]);
					 											$l8 = $this->getLevel($m8["openid"]);
					 											if ($l8["levelname"]=="消费股东" or $l8["levelname"]=="公司合伙人") {
					 												if ($cinfo["realprice"]>$l8["ling1down"] && $cinfo["realprice"]<=$l8["ling1up"]) {
																		$money = $l8["ling1money"];
																	}
																	if ($cinfo["realprice"]>$l8["ling2down"] && $cinfo["realprice"]<=$l8["ling2up"]) {
																		$money = $l8["ling2money"];
																	}
																	if ($cinfo["realprice"]>$l8["ling3down"] && $cinfo["realprice"]<=$l8["ling3up"]) {
																		$money = $l8["ling3money"];
																	}
																	if ($cinfo["realprice"]>$l8["ling4down"] && $cinfo["realprice"]<=$l8["ling4up"]) {
																		$money = $l8["ling4money"];
																	}
																	$commissionlead["level5"]= $money;
																}
					 										}
					 									}
					 								}
								...
								// 开始处理非套餐区的业务
									...
									$commissions["level1"] = round($l1["ntc1"] * $price,2);///////////////////////////
									if(!empty($m1["agentid"])){
										$m2 = m("member")->getMember($m1["agentid"]);
										$l2 = $this->getLevel($m2["openid"]);
										$commissions["level2"] = round($l2["ntc2"] * $price,2) ;////////////////////////////////////
										//show_json()
										if(!empty($m2["agentid"])){
											$m3 = m("member")->getMember($m2["agentid"]);
											$l3 = $this->getLevel($m2["openid"]);
											$commissions["level3"] = round($l3["ntc3"] * $price,2) ;
											if(!empty($m3["agentid"])){
												$m4 = m("member")->getMember($m3["agentid"]);
												$l4 = $this->getLevel($m3["openid"]);
												$commissionlead["level1"] = round($l4["ntc4"] * $price,2) ;
												if(!empty($m4["agentid"])){
													$m5 = m("member")->getMember($m4["agentid"]);
													$l5 = $this->getLevel($m5["openid"]);
													$commissionlead["level2"] = round($l5["ntc5"] * $price,2);
													if(!empty($m5["agentid"])){
														$m6 = m("member")->getMember($m5["agentid"]);
														$l6 = $this->getLevel($m6["openid"]);
														$commissionlead["level3"] = round($l6["ntc6"] * $price,2);
														if(!empty($m6["agentid"])){
															$m7 = m("member")->getMember($m6["agentid"]);
															$l7 = $this->getLevel($m7["openid"]);
															$commissionlead["level4"] = round($l7["ntc7"] * $price,2);
															if(!empty($m7["agentid"])){
																$m8 = m("member")->getMember($m7["agentid"]);
																$l8 = $this->getLevel($m8["openid"]);
																$commissionlead["level5"] = round($l8["ntc8"] * $price,2) ;
															}
														}
													}
												}
						...
						pdo_update("ewei_shop_order_goods", array( "commission1" => iserializer($cinfo["commission1"]), "commission2" => iserializer($cinfo["commission2"]), "commission3" => iserializer($cinfo["commission3"]), "commissions" => iserializer($commissions), "nocommission" => $cinfo["nocommission"],"commissionlead"=>iserializer($commissionlead)), array( "id" => $cinfo["id"] ));
					}
				}
				unset($cinfo);
			}
		...
		public function GetTotalforme($agentid){
        	...
        	array( ":agid" => $agentid));
      		if(count($allmember)<1) return $totleforme;
      		$implodestr = '';
			foreach ($allmember as $c) {
				$implodestr = $implodestr . '"'. $c["openid"] . '",';
			}
			$implodestr = substr($implodestr,0,strlen($implodestr)-1);
        	...
        	. tablename("ewei_shop_order") . " o " .
        		...
        		" where o.openid in( " . $implodestr . ") and o.status > 0 and oc.pcate = 1176 " );
      		$totleforme+=$totle["ordermoney"];
			do {
			  	//根据下级的openid获得id
			  	$jiedianrenid = pdo_fetchall('select id from  ' . tablename("ewei_shop_member").' where openid in (' . $implodestr .')');
			  	if (count($jiedianrenid) <= 0) {
			    	return $totleforme;
			  	}
        	 	//如果数量为0，说明没有下级，直接返回     
        		//制作拼接字符串
			  	$implodestr1 = '';
			  	$cengjilist = $cengjilist . "-" . $cengji . "-";
			  	foreach ($jiedianrenid as $c) {
			    	$implodestr1 = $implodestr1 .  $c["id"] . ',';
			    	$cengjilist  = $cengjilist . $c["id"] . "=";
			  	}
				$implodestr1 = substr($implodestr1,0,strlen($implodestr1)-1);
			  	//根据id查出下级的openid
			  	$allmember = pdo_fetchall("select openid from " . tablename("ewei_shop_member") .
	        	" where jiedianrenid in (" . $implodestr1 .")");
				if (count($allmember)<=0 or strlen($implodestr1)<3) {
					return $totleforme;
				}
			 	$implodestr = '';
			  	foreach ($allmember as $c) {
				...
				$totle = pdo_fetch("select sum(og.realprice) as ordermoney from "
			  	...
			  	$temp1= isset($totle["ordermoney"]) ? $totle["ordermoney"] : 0;
				$totleforme += $temp1 ;
        		$cengji++;
		...
		//获取某人的上周的累计业绩
		public function GetTotalformeByWeek($agnetid){
			global $_W;
			$totalforme = 0;
        	...
        	pdf_update("ewei_shop_member",array("weeksum"=>$totalforme,"weeksumnum"=>date('W')),array("id"=>$angentid));
			return $totalforme;
		}
		//计算此人应该得多少周分红
		public function GetWeekGlons($id){
			global $_W;
			$member = m("member")->getMember($agentid);
			$total = 0;
			// 如果已经计算过了，则直接返回结果
			if($member["weekglonsnum"]==date('w')) return $member["weekmoney"];
			//取得所有下级
			$allmember = pdo_fetchall("select openid from " . tablename("ewei_shop_member") .
        		" where jiedianrenid =:agid and isagent=1 and status=1 ",
        		array( ":agid" => $agentid));
			//当用户从来没有满足分红条件时
			if ($member["weeklevel"]==0) {
				//是否满足升级条件，如果满足，那累计业绩的钱，如果不满足，直接返回0
				if (!$this->LevelUp($allmember,100000,2)) {
					return 0;
				}else{
					pdo_update("ewei_shop_member",array("weeklevel"=>1),array("id"=>$agentid));
					foreach ($allmember as $onemember) {
						$allnextmember = pdo_fetchall("select openid from " . tablename("ewei_shop_member") .
        					" where jiedianrenid =:agid and isagent=1 and status=1 ",
        					array( ":agid" => $onemember["id"]));
						if (!$this->LevelUp($allnextmember,100000,2)) {
							$total+=$this->GetTotalformeByWeek($onemember["id"])*0.03;
						}
					}
				}
			}
			// 当用户已经满足10万的条件时
			if ($member["weeklevel"]==1) {
				//是否满足升级条件，如果满足，那累计业绩的钱，如果不满足，拿新增业绩的提成
				if (!$this->LevelUp($allmember,300000,3)) {
					foreach ($allmember as $onemember) {
						$allnextmember = pdo_fetchall("select openid from " . tablename("ewei_shop_member") .
        					" where jiedianrenid =:agid and isagent=1 and status=1 ",
        					array( ":agid" => $onemember["id"]));
						if (!$this->LevelUp($allnextmember,100000,2)) {
							$total+=$this->GetNewTotalformeByWeek($onemember["id"])*0.03;
						}
					}
				}else{
					pdo_update("ewei_shop_member",array("weeklevel"=>2),array("id"=>$agentid));
					foreach ($allmember as $onemember) {
						$allnextmember = pdo_fetchall("select openid from " . tablename("ewei_shop_member") .
        					" where jiedianrenid =:agid and isagent=1 and status=1 ",
        					array( ":agid" => $onemember["id"]));
						if (!$this->LevelUp($allnextmember,300000,3)) {
							$total+=$this->GetTotalformeByWeek($onemember["id"])*0.03;
						}
					}
				}
			...
			// 当用户已经满足30万的条件时
			if ($member["weeklevel"]==2) {
				//是否满足升级条件，如果满足，那累计业绩的钱，如果不满足，拿新增业绩的提成
				if (!$this->LevelUp($allmember,1000000,4)) {
					foreach ($allmember as $onemember) {
						$allnextmember = pdo_fetchall("select openid from " . tablename("ewei_shop_member") .
        					" where jiedianrenid =:agid and isagent=1 and status=1 ",
        					array( ":agid" => $onemember["id"]));
					...
					foreach ($allmember as $onemember) {
						$allnextmember = pdo_fetchall("select openid from " . tablename("ewei_shop_member") .
        					...
        					array( ":agid" => $onemember["id"]));
						if (!$this->LevelUp($allnextmember,300000,3)) {
							$total+=$this->GetTotalformeByWeek($onemember["id"])*0.04;
						}
					}
				}
			}
			// 当用户已经满足100万的条件时
			if ($member["weeklevel"]==3) {
				foreach ($allmember as $onemember) {
					$allnextmember = pdo_fetchall("select openid from " . tablename("ewei_shop_member") .
        				" where jiedianrenid =:agid and isagent=1 and status=1 ",
        				array( ":agid" => $onemember["id"]));
					if ($this->LevelUp($allnextmember,1000000,4)) {
						$total+=0;
					} else if($this->LevelUp($allnextmember,300000,3)){
						$total+=$this->GetNewTotalformeByWeek($onemember["id"])*0.04;
					} else if($this->LevelUp($allnextmember,100000,2)){
						$total+=$this->GetNewTotalformeByWeek($onemember["id"]) * 0.07;
					} else{
						$total+=$this->GetNewTotalformeByWeek($onemember["id"]) * 0.1;
					}
				}				
			}
		...
		//判断是否满足升级的条件，level表示升级的钱数，num表示升级的个数
		public function LevelUp($allmember,$level=0,$num=0){
			if(empty($allmember)) return false;
			$tempnum = 0;
			foreach ($allmember as $onemember) {
				if($this->GetTotalformeByWeek($onemember["id"])>$level) $tempnum++;
		...
		public function getInfo($openid, $options = NULL)
		{
			...
			if( 2 <= $level && 0 < $level1 )
			{
                ...
                if( in_array("total", $options) )
                {
                    $level2_commissions = pdo_fetchall("select og.commission3,og.commissions from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where o.agentid in( " . implode(",", array_keys($level1_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
                    foreach( $level2_commissions as $c )
                    {
                        ...
                        {
                            $commission_total += (isset($commissions["level2"]) ? floatval($commissions["level2"]) : 0);
                        }
                    }
                }
				if (in_array('ok', $options)) {
			...
			if (0<$level3) {
				if( in_array("total", $options) )
				{
				  $level4_commissions = pdo_fetchall("select og.commissionlead from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where o.agentid in( " . implode(",", array_keys($level3_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
				    foreach ($level4_commissions as $c )
				    {
						$commissionlead =iunserializer($c['commissionlead']);
					 	if (!empty($commissionlead))
						{
							$commission_total += ((isset($commissionlead['level1']) ? $commissionlead['level1'] : 0));
						}
				   }
				}
				...
				//v1.000 start
				if (in_array('ok', $options)) {
					$level3_commissions = pdo_fetchall('select og.commissionlead   from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level3_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and og.statuslead1=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level3_commissions as $c ) {
						$commissionlead =iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_ok += ((isset($commissionlead['level1']) ? $commissionlead['level1'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('lock', $options)) {
					$level4_commissions1 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level3_agentids)) . ')  and o.status>=3 and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.statuslead1=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level4_commissions1 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_lock += ((isset($commissionlead['level1']) ? $commissionlead['level1'] :0));
						}
					}
				}
				//v1.000 end
				//v1.000start
				if (in_array('apply', $options)) {
					$level4_commissions2 = pdo_fetchall('select og.commissionlead from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level3_agentids)) . ')  and o.status>=3 and og.statuslead1=1 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level4_commissions2 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_apply += ((isset($commissionlead['level1']) ? $commissionlead['level1'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('check', $options)) {
					$level4_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level3_agentids)) . ')  and o.status>=3 and og.statuslead1=2 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level4_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_check += ((isset($commissionlead['level1']) ? $commissionlead['level1'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('pay', $options)) {
					$level4_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level3_agentids)) . ')  and o.status>=3 and og.statuslead1=3 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level4_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_pay += ((isset($commissionlead['level1']) ? $commissionlead['level1'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('wait', $options)) {
					$level4_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level3_agentids)) . ')  and ( o.status=1 or o.status=2) and og.statuslead1=0 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level4_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_wait += ((isset($commissionlead['level1']) ? $commissionlead['level1'] : 0));
						}
					}
				}
				//v1.000 end
				...
				if (in_array('fail', $options)) {
					...
					foreach ($level4_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_fail += ((isset($commissionlead['level1']) ? $commissionlead['level1'] : 0));
						}
					}
				}
				//v1.000 end
				...
				$level4_agentids = pdo_fetchall("select id from " . tablename("ewei_shop_member") . " where uniacid=:uniacid and agentid in( " . implode(",", array_keys($level3_agentids)) . ") and isagent=1 and status=1", array( ":uniacid" => $_W["uniacid"] ), "id");
		  ...
		  if (0<$level4) {
				  ...
				  $level5_commissions = pdo_fetchall("select og.commissionlead from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where o.agentid in( " . implode(",", array_keys($level4_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
				    foreach ($level5_commissions as $c )
				    {
						$commissionlead =iunserializer($c['commissionlead']);
					 	if (!empty($commissionlead))
						{
							$commission_total += ((isset($commissionlead['level2']) ? $commissionlead['level2'] : 0));
						}
				   }
				}
				// $agentcount += $level4;
				//v1.000 start
				if (in_array('ok', $options)) {
					$level5_commissions = pdo_fetchall('select og.commissionlead   from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level4_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and og.statuslead2=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level5_commissions as $c ) {
						$commissionlead =iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_ok += ((isset($commissionlead['level2']) ? $commissionlead['level2'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('lock', $options)) {
					$level5_commissions1 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level4_agentids)) . ')  and o.status>=3 and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.statuslead2=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level5_commissions1 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_lock += ((isset($commissionlead['level2']) ? $commissionlead['level2'] :0));
						}
					}
				}
				//v1.000 end
				//v1.000start
				if (in_array('apply', $options)) {
					$level5_commissions2 = pdo_fetchall('select og.commissionlead from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level4_agentids)) . ')  and o.status>=3 and og.statuslead2=1 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level5_commissions2 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_apply += ((isset($commissionlead['level2']) ? $commissionlead['level2'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('check', $options)) {
					$level5_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level4_agentids)) . ')  and o.status>=3 and og.statuslead2=2 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level5_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_check += ((isset($commissionlead['level2']) ? $commissionlead['level2'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('pay', $options)) {
					$level5_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level4_agentids)) . ')  and o.status>=3 and og.statuslead2=3 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level5_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_pay += ((isset($commissionlead['level2']) ? $commissionlead['level2'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('wait', $options)) {
					$level5_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level4_agentids)) . ')  and ( o.status=1 or o.status=2) and og.statuslead2=0 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level5_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_wait += ((isset($commissionlead['level2']) ? $commissionlead['level2'] : 0));
						}
					}
				}
				//v1.000 end
				...
				if (in_array('fail', $options)) {
					...
					foreach ($level3_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_fail += ((isset($commissionlead['level2']) ? $commissionlead['level2'] : 0));
						}
					}
				}
				//v1.000 end
				$level5_agentids = pdo_fetchall("select id from " . tablename("ewei_shop_member") . " where uniacid=:uniacid and agentid in( " . implode(",", array_keys($level4_agentids)) . ") and isagent=1 and status=1", array( ":uniacid" => $_W["uniacid"] ), "id");
			...
			if (0<$level5) {
				if( in_array("total", $options) )
				{
				  $level6_commissions = pdo_fetchall("select og.commissionlead from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where o.agentid in( " . implode(",", array_keys($level5_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
				    foreach ($level6_commissions as $c )
				    {
						$commissionlead =iunserializer($c['commissionlead']);
					 	if (!empty($commissionlead))
						{
							$commission_total += ((isset($commissionlead['level3']) ? $commissionlead['level3'] : 0));
						}
				   }
				}
				// $agentcount += $level4;
				//v1.000 start
				if (in_array('ok', $options)) {
					$level6_commissions = pdo_fetchall('select og.commissionlead   from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level5_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and og.statuslead3=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level6_commissions as $c ) {
						$commissionlead =iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_ok += ((isset($commissionlead['level3']) ? $commissionlead['level3'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('lock', $options)) {
					$level6_commissions1 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level5_agentids)) . ')  and o.status>=3 and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.statuslead3=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level6_commissions1 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_lock += ((isset($commissionlead['level3']) ? $commissionlead['level3'] :0));
						}
					}
				}
				//v1.000 end
				//v1.000start
				if (in_array('apply', $options)) {
					$level6_commissions2 = pdo_fetchall('select og.commissionlead from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level5_agentids)) . ')  and o.status>=3 and og.statuslead3=1 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level6_commissions2 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_apply += ((isset($commissionlead['level3']) ? $commissionlead['level3'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('check', $options)) {
					$level6_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level5_agentids)) . ')  and o.status>=3 and og.statuslead3=2 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level6_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_check += ((isset($commissionlead['level3']) ? $commissionlead['level3'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('pay', $options)) {
					$level6_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level5_agentids)) . ')  and o.status>=3 and og.statuslead3=3 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level6_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_pay += ((isset($commissionlead['level3']) ? $commissionlead['level3'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('wait', $options)) {
					$level6_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level5_agentids)) . ')  and ( o.status=1 or o.status=2) and og.statuslead3=0 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level6_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_wait += ((isset($commissionlead['level3']) ? $commissionlead['level3'] : 0));
						}
					}
				}
				//v1.000 end
				...
				if (in_array('fail', $options)) {
					...
					foreach ($level6_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_fail += ((isset($commissionlead['level3']) ? $commissionlead['level3'] : 0));
						}
					}
				}
				//v1.000 end
				$level6_agentids = pdo_fetchall("select id from " . tablename("ewei_shop_member") . " where uniacid=:uniacid and agentid in( " . implode(",", array_keys($level5_agentids)) . ") and isagent=1 and status=1", array( ":uniacid" => $_W["uniacid"] ), "id");
			...
			if (0<$level6) {
				if( in_array("total", $options) )
				{
				  $level7_commissions = pdo_fetchall("select og.commissionlead from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where o.agentid in( " . implode(",", array_keys($level6_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
				    foreach ($level7_commissions as $c )
				    {
						$commissionlead =iunserializer($c['commissionlead']);
					 	if (!empty($commissionlead))
						{
							$commission_total += ((isset($commissionlead['level4']) ? $commissionlead['level4'] : 0));
						}
				   }
				}
				// $agentcount += $level4;
				//v1.000 start
				if (in_array('ok', $options)) {
					$level7_commissions = pdo_fetchall('select og.commissionlead   from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level6_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and og.statuslead4=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level7_commissions as $c ) {
						$commissionlead =iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_ok += ((isset($commissionlead['level4']) ? $commissionlead['level4'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('lock', $options)) {
					$level7_commissions1 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level6_agentids)) . ')  and o.status>=3 and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.statuslead4=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level7_commissions1 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_lock += ((isset($commissionlead['level4']) ? $commissionlead['level4'] :0));
						}
					}
				}
				//v1.000 end
				//v1.000start
				if (in_array('apply', $options)) {
					$level7_commissions2 = pdo_fetchall('select og.commissionlead from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level6_agentids)) . ')  and o.status>=3 and og.statuslead4=1 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level7_commissions2 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_apply += ((isset($commissionlead['level4']) ? $commissionlead['level4'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('check', $options)) {
					$level7_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level6_agentids)) . ')  and o.status>=3 and og.statuslead4=2 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level7_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_check += ((isset($commissionlead['level4']) ? $commissionlead['level4'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('pay', $options)) {
					$level7_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level6_agentids)) . ')  and o.status>=3 and og.statuslead4=3 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level7_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_pay += ((isset($commissionlead['level4']) ? $commissionlead['level4'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('wait', $options)) {
					$level7_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level6_agentids)) . ')  and ( o.status=1 or o.status=2) and og.statuslead4=0 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level7_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_wait += ((isset($commissionlead['level4']) ? $commissionlead['level4'] : 0));
						}
					}
				}
				//v1.000 end
				...
				if (in_array('fail', $options)) {
					...
					foreach ($level7_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_fail += ((isset($commissionlead['level4']) ? $commissionlead['level4'] : 0));
						}
					}
				}
				//v1.000 end
				$level7_agentids = pdo_fetchall("select id from " . tablename("ewei_shop_member") . " where uniacid=:uniacid and agentid in( " . implode(",", array_keys($level6_agentids)) . ") and isagent=1 and status=1", array( ":uniacid" => $_W["uniacid"] ), "id");
				$level7 = count($level7_agentids);
				// $agentcount += $level5;
			}
			if (0<$level7) {
				if( in_array("total", $options) )
				{$level8_commissions = pdo_fetchall("select og.commissionlead from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on o.id = og.orderid" . " where o.agentid in( " . implode(",", array_keys($level7_agentids)) . ")  and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0", array( ":uniacid" => $_W["uniacid"] ));
				    foreach ($level8_commissions as $c )
				    {
						$commissionlead =iunserializer($c['commissionlead']);
					 	if (!empty($commissionlead))
						{
							$commission_total += ((isset($commissionlead['level5']) ? $commissionlead['level5'] : 0));
						}
				   }
				}
				// $agentcount += $level4;
				//v1.000 start
				if (in_array('ok', $options)) {
					$level8_commissions = pdo_fetchall('select og.commissionlead   from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level7_agentids)) . ')  and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3 and og.statuslead5=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level8_commissions as $c ) {
						$commissionlead =iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_ok += ((isset($commissionlead['level5']) ? $commissionlead['level5'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('lock', $options)) {
					$level8_commissions1 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level7_agentids)) . ')  and o.status>=3 and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.statuslead5=0  and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level8_commissions1 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_lock += ((isset($commissionlead['level5']) ? $commissionlead['level5'] :0));
						}
					}
				}
				//v1.000 end
				//v1.000start
				if (in_array('apply', $options)) {
					$level8_commissions2 = pdo_fetchall('select og.commissionlead from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level7_agentids)) . ')  and o.status>=3 and og.statuslead5=1 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level8_commissions2 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_apply += ((isset($commissionlead['level5']) ? $commissionlead['level5'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('check', $options)) {
					$level8_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level7_agentids)) . ')  and o.status>=3 and og.statuslead5=2 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level8_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_check += ((isset($commissionlead['level5']) ? $commissionlead['level5'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('pay', $options)) {
					$level8_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level7_agentids)) . ')  and o.status>=3 and og.statuslead5=3 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level8_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_pay += ((isset($commissionlead['level5']) ? $commissionlead['level5'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('wait', $options)) {
					$level8_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level7_agentids)) . ')  and ( o.status=1 or o.status=2) and og.statuslead5=0 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level8_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_wait += ((isset($commissionlead['level5']) ? $commissionlead['level5'] : 0));
						}
					}
				}
				//v1.000 end
				//v1.000 start
				if (in_array('fail', $options)) {
					$level8_commissions3 = pdo_fetchall('select og.commissionlead  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . ' where o.agentid in( ' . implode(',', array_keys($level7_agentids)) . ')  and o.status=3 and og.statuslead5=-1 and og.nocommission=0 and o.uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
					foreach ($level8_commissions3 as $c ) {
						$commissionlead = iunserializer($c['commissionlead']);
						if (!empty($commissionlead)) {
							$commission_fail += ((isset($commissionlead['level5']) ? $commissionlead['level5'] : 0));
						}
					}
				}
				//v1.000 end
            ...
            //v1.009 start
			$member["totleforme"] = $totleforme;
            //v1.009 end
			// v1.002 start
				// v1.015 可提现月分红
			        //$member["monthymoney"]  = ( empty($member["monthmoney"]) || $member["monthnum"]==date("m") ) ? 0 : $member["monthmoney"] ;
                    $member['monthymoney'] = $member['monthmoney_old'];
                // v1.015
            //v1.009 start
            $member["commission_total"] = $commission_total + $member["monthmoney_ok"] + $member["monthmoney_old"];
            //v1.009 end
				// v1.003 start
            /**
             * commission_ok     可提现
             * commission_pay    已提现
             * commission_apply  已申请奖金
             * commission_check  待打款奖金
             * commission_fail   无效奖金
             * monthmoney  上月月分红钱数
             * monthmoney_old  往月分红累计
             * monthmoney_zhuangtai  月分红提现状态
             * monthmoney_ok  已提现月分红累计
             * monthmoney_ok_time  最近一次月分红提现时间
             ...
             * $member['order_w'] 未结算订单
             **/
            //c1.000start 定义提现详情显示
            $member['ca'] = pdo_fetchall('select * from '.tablename('ims_ewei_shop_commission_apply')."  where mid = ".$member['id']);
            //查询用户提现表所有信息
			...
			$member["level4_agentids"] = $level4_agentids;
			$member["level5"] = $level5;
			$member["level5_agentids"] = $level5_agentids;
			$member["level6"] = $level6;
			$member["level6_agentids"] = $level6_agentids;
			$member["level7"] = $level7;
			$member["level7_agentids"] = $level7_agentids;
			$member["agenttime"] = date("Y-m-d H:i", $member["agenttime"]);
			$member["weekmoney"] = empty($member["weekmoney"])?0:$member["weekmoney"];
			$member["montymoney"] = empty($member["monthmoney"])?0:$member["monthmoney"];
		...
		public function sendMessage($openid = "", $data = array( ), $message_type = "") 
		{
			...
			if( $message_type == TM_COMMISSION_AGENT_NEW && empty($usernotice["commission_agent_new"]) ) 
			{
				if( $tm["is_advanced"] ) 
				{
					...
					$message = array( "first" => array( "value" => "亲爱的" . $data["nickname"] . "，您新增了一个下级分销商" . $data["nickname"], "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => "下级付款通知", "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => "您的下级" . $data["nickname"] . "已经成功付款", "color" => "#000000" ), "remark" => array( "value" => "\n感谢您的支持", "color" => "#000000" ) );
					$toopenid = $openid;
				...
				else 
				{
					...
					$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_agent_newtitle"]) ? $tm["commission_agent_newtitle"] : "新增下级分销商通知"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
			...
			else 
			{
				if( $message_type == TM_COMMISSION_ORDER_PAY && empty($usernotice["commission_order_pay"]) ) 
				{
					if( $tm["is_advanced"] ) 
					{
						...
						$message = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "，您的下级分销商" . $data["nickname"] . "已付款", "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => "下级分销商付款通知", "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => "您的下级分销商" . $data["nickname"] . "已经成功付款", "color" => "#000000" ), "remark" => array( "value" => "\n感谢您的支持", "color" => "#000000" ) );
						$toopenid = $openid;
						$datas[] = array( "name" => "昵称", "value" => $data["nickname"] );
						$datas[] = array( "name" => "下级昵称", "value" => $data["nickname"] );
					...
					else 
					{
						...
						$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_order_paytitle"]) ? $tm["commission_order_paytitle"] : "下线分销商付款通知") ), "keyword3" => array( "value" => $message ) );
				...
				else 
				{
					if( $message_type == TM_COMMISSION_ORDER_FINISH && empty($usernotice["commission_order_finish"]) ) 
					{
						if( $tm["is_advanced"] ) 
						{
							...
							$message = array( "first" => array( "value" => "亲爱的" . $member["nickname"] . "，您的下级分销商" . $data["nickname"] . "已确认收货", "color" => "#ff0000" ), "keyword1" => array( "title" => "业务类型", "value" => "会员通知", "color" => "#000000" ), "keyword2" => array( "title" => "业务状态", "value" => "下级分销商收货通知", "color" => "#000000" ), "keyword3" => array( "title" => "业务内容", "value" => "您的下级分销商" . $data["nickname"] . "已经确认收货", "color" => "#000000" ), "remark" => array( "value" => "\n感谢您的支持", "color" => "#000000" ) );
							$toopenid = $openid;
						...
						else 
						{
							...
							$message = str_replace("[订单金额]", $data["price"], $message);
							$message = str_replace("[商品详情]", $data["goods"], $message);
							$msg = array( "keyword1" => array( "value" => "会员通知", "color" => "#73a68d" ), "keyword2" => array( "value" => (!empty($tm["commission_order_finishtitle"]) ? $tm["commission_order_finishtitle"] : "下线分销商确认收货通知"), "color" => "#73a68d" ), "keyword3" => array( "value" => $message, "color" => "#73a68d" ) );
			...
			if( $tm["is_advanced"] && ($tag == "commission_agent_new" || $tag == "commission_order_pay" || $tag == "commission_order_finish") ) 
			{
				...
				foreach( $res as $key => $value ) 
				{
					if( $key == 0 ) 
					{
						...
						{
							$text = "您的二级分销商" . $data["nickname"] . "已付款！";
						}
						else 
						{
							if( $tag == "commission_order_finish" ) 
							{
								$text = "您的二级分销商" . $data["nickname"] . "已确认收货！";
							}
						}
					}
					if( $key == 1 ) 
					{
						...
						if( $tag == "commission_order_pay" ) 
						{
							$text = "您的三级分销商" . $data["nickname"] . "已付款！";
						}
						else 
						{
							if( $tag == "commission_order_finish" ) 
							{
								$text = "您的三级下线分销商" . $data["nickname"] . "已确认收货！";
							}
						}
					}
