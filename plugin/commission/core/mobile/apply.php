...
class Apply_EweiShopV2Page extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		//v1.000 start
		if (0 < $member['level3']) {
			$level3_orders = pdo_fetchall('select distinct o.id from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level3_agentids'])) . (')  and o.status>=3  and  og.statuslead1=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ')   and o.uniacid=:uniacid  group by o.id'), array(':uniacid' => $_W['uniacid']));
			...
			foreach ($level3_orders as $o) {
				...
				$orderids[] = array('orderid' => $o['id'], 'level' => 4);
			}
		}
		if (0 < $member['level4']) {
			$level4_orders = pdo_fetchall('select distinct o.id from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level4_agentids'])) . (')  and o.status>=3  and  og.statuslead2=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ')   and o.uniacid=:uniacid  group by o.id'), array(':uniacid' => $_W['uniacid']));
			...
			foreach ($level4_orders as $o) {
				...
				$orderids[] = array('orderid' => $o['id'], 'level' => 5);
			}
		}
		if (0 < $member['level5']) {
			$level5_orders = pdo_fetchall('select distinct o.id from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level5_agentids'])) . (')  and o.status>=3  and  og.statuslead3=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ')   and o.uniacid=:uniacid  group by o.id'), array(':uniacid' => $_W['uniacid']));
			...
			foreach ($level5_orders as $o) {
				...
				$orderids[] = array('orderid' => $o['id'], 'level' => 6);
			}
		}
		if (0 < $member['level6']) {
			$level6_orders = pdo_fetchall('select distinct o.id from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level6_agentids'])) . (')  and o.status>=3  and  og.statuslead4=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ')   and o.uniacid=:uniacid  group by o.id'), array(':uniacid' => $_W['uniacid']));
			...
			foreach ($level6_orders as $o) {
				...
				$orderids[] = array('orderid' => $o['id'], 'level' => 7);
			}
		}
		if (0 < $member['level7']) {
			$level7_orders = pdo_fetchall('select distinct o.id from ' . tablename('ewei_shop_order') . ' o ' . ' left join  ' . tablename('ewei_shop_order_goods') . ' og on og.orderid=o.id ' . ' where o.agentid in( ' . implode(',', array_keys($member['level7_agentids'])) . (')  and o.status>=3  and  og.statuslead5=0 and og.nocommission=0 and (' . $time . ' - o.finishtime > ' . $day_times . ')   and o.uniacid=:uniacid  group by o.id'), array(':uniacid' => $_W['uniacid']));
		...
		foreach ($orderids as $o) {
			...
			foreach ($goods as $g) {
				...
				//show_json(1,$commission_ok);
				// v1.000 start
				if ($o['level'] == 4 && $g['statuslead1'] == 0) {

					$commissionlead = iunserializer($g['commissionlead']);
					$commission_ok += isset($commissionlead['level1']) ? floatval($commissionlead['level1']) :0;
					
				}
				if ($o['level'] == 5 && $g['statuslead2'] == 0) {
					$commissionlead = iunserializer($g['commissionlead']);
					$commission_ok += isset($commissionlead['level2']) ? floatval($commissionlead['level2']) :0;
					
				}
				if ($o['level'] == 6 && $g['statuslead3'] == 0) {
					$commissionlead= iunserializer($g['commissionlead']);
					$commission_ok += isset($commissionlead['level3']) ? floatval($commissionlead['level3']) :0;
				}
				if ($o['level'] == 7 && $g['statuslead4'] == 0) {
					$commissionlead= iunserializer($g['commissionlead']);
					$commission_ok += isset($commissionlead['level4']) ? floatval($commissionlead['level4']) :0;
				}
				if ($o['level'] == 8 && $g['statuslead5'] == 0) {
					$commissionlead= iunserializer($g['commissionlead']);
					$commission_ok += isset($commissionlead['level5']) ? floatval($commissionlead['level5']) :0;
				}
				//v1.000 end
			}
		}
		...
		// v1.003 start
		//可体现金额
		//$commission_ok +=$member["commission_old_no"];
		//v1.009 start
				$commission_ok += $member["commission_apply_can"];
			//v1.009 end
		// v1.003 end
...
// || empty($orderids)
			...
			foreach ($orderids as $o) {
				...
				if ($o['level']<4) {
					...
					pdo_update('ewei_shop_order_goods', array('statuslead' . ($o['level']-3) => 1), array('orderid' => $o['orderid'], 'uniacid' => $_W['uniacid']));
				}
				//v1.000 end 修改
			...
			//v1.009 start 判断提现类型，如果是提现到余额没有手续费
            ...
            if( $apply['type'] != 0 ){
                ...
                $apply['realmoney'] = $commission_ok;
                $apply['deductionmoney'] = 0;
                $apply['charge'] = 0;
            }
			...
			//v1.009 start
			//获取申请提现的月分红奖金
            $apply['monthmoney'] = $member['monthmoney_zhuangtai'] == 0 ? $member['monthmoney_old'] : 0 ;
            //v1.009 end
			...
			//v1.009 start
				if( $member['monthmoney_zhuangtai'] > 0 ){
					...
					pdo_update('ewei_shop_member',array('monthmoney_zhuangtai' => 1 ),array('id'=>$apply['mid']));//提交之后将月分红放到monthmoney_old_on
					pdo_update('ewei_shop_member',array('monthmoney_old_no' => $apply['monthmoney'] ),array('id'=>$apply['mid']));
				}
				//v1.009 end
			//v1.003 end
