...
class Register_EweiShopV2Page extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		$mid = intval($_GPC['mid']);
		$jdmid = intval($_PGC['jdmid']);
		$agent = false;

		if (!empty($member['fixagentid'])) {
			$mid = $member['agentid'];
			$jdmid = $memeber['jiedianrenid'];

			if (!empty($mid)) {
				$agent = m('member')->getMember($member['agentid']);
				$jiedianren = m('member')->getMember($member['jiedianrenid']);
			}
		}
		else if (!empty($member['agentid'])) {
			$mid = $member['agentid'];
			$jdmid = $member['jiedianrenid'];
			$agent = m('member')->getMember($member['agentid']);
			$jiedianren = m('member')->getMember($member['jiedianrenid']);
		}
			...
			$jieidanren = m('member')->getMember($member['jieidanrenid']);
		}
		else {
			if (!empty($mid)) {
			...
			if(!empty($jdmid)){
				$jiedianren = m('member')->getMember($jdmid);
			}
		}
		...
		if ($_W['ispost']) {
			...
			$jdmid = intval($_GPC['jiedianrenid']);
			...
			$isindown = $this->isIndown($mid,$jdmid);
			//show_json(0,$jdmid);
			 if($isindown==0){
			 	show_json(0,'接点人未在推荐人三层范围内或不存在');
		    }
			...
			if ($template_flag == 1) {
				...
				$data = array('isagent' => 1, 'agentid' => $mid, 'status' => $become_check, 'realname' => $_GPC['realname'], 'mobile' => $_GPC['mobile'], 'weixin' => $_GPC['weixin'], 'agenttime' => $become_check == 1 ? time() : 0,'jiedianrenid'=>intval($_GPC['jiedianrenid']));
				//show_json(1,array('jiedianrenid'=>intval($_GPC['jiedianrenid'])));
	...
	protected function isIndown($agentid,$jiedianrenid)
	{
		if($agentid == $jiedianrenid){
			return 1;
		}
		$m1 = m("member")->getMember($jiedianrenid);
		if ($m1["agentid"]==$agentid) {
			return 1;
		}
		$m2 = m("member")->getMember($m1["agentid"]);
		if ($m2["agentid"]==$agentid) {
			return 1;
		}
		$m3 = m("member")->getMember($m2["agentid"]);
		if ($m3["agentid"]==$agentid) {
			return 1;
		}
		return 0;
	}
