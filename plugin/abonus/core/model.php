...
if (!(class_exists('AbonusModel'))) {
...

	{
...

		{
			...
			//$orders = pdo_fetchall('select id,openid,price,grprice,addressid, address from ' . tablename('ewei_shop_order') . ' where uniacid=' . $_W['uniacid'] . ' and addressid<>0 and status=3 and isabonus=0 and finishtime + ' . $settletimes . '>= ' . $starttime . ' and  finishtime + ' . $settletimes . '<=' . $endtime, array(), 'id');

			//$orders = pdo_fetchall('select a.id, a.openid, a.price, addressid, a.address from ' . tablename('ewei_shop_order') . 'a  where a.uniacid=' . $_W['uniacid'] . ' and a.addressid<>0 and a.status=3 and a.isabonus=0 and a.finishtime + ' . $settletimes . ' >= ' . $starttime . ' and  a.finishtime + ' . $settletimes . ' <= ' . $endtime, array(), 'a.id');
			//v1.004 start
			$orders = pdo_fetchall('select a.id, a.openid, a.price, a.grprice, a.addressid, a.address, b.orderid, b.goodsid, c.id, c.pcate  from ' . tablename('ewei_shop_order') . 'a left join' . tablename('ewei_shop_order_goods') . ' b on a.id = b.orderid left join '. tablename('ewei_shop_goods') .' c  on b.goodsid = c.id   where a.uniacid=' . $_W['uniacid'] . ' and a.addressid<>0 and a.status=3 and a.isabonus=0 and a.finishtime + ' . $settletimes . '>= ' . $starttime . ' and  a.finishtime + ' . $settletimes . '<=' . $endtime, array(), 'a.id');
			//v1.004 end
...
//v1.004 end
			...
			foreach ($aagents as &$a ) {
				...
				if (empty($a['aagentlevel']) || (($a['bonus1'] == NULL) && ($a['bonus2'] == NULL) && ($a['bonus3'] == NULL) && ($a['bonus4'] == NULL) && ($a['bonus5'] == NULL) && ($a['bonus6'] == NULL))) {
			...
			foreach ($orders as $o ) {
				...
				foreach ($aagents as &$a ) {
					...
					!(isset($a['bonusmoney4'])) && ($a['bonusmoney4'] = 0);

					!(isset($a['bonusmoney5'])) && ($a['bonusmoney5'] = 0);

					!(isset($a['bonusmoney6'])) && ($a['bonusmoney6'] = 0);
...
//v1.004 start
					...
					if ($a['aagenttype'] == 1) {
						...
						if (in_array($address['province'], $a['aagentprovinces'])) {
							...
							if($o['pcate']==1176){
								$bonusmoney1 = round(($o['grprice'] * $a['bonus1']) / 100, 2);
							}else{
								$bonusmoney1 = round(($o['grprice'] * $a['bonus4']) / 100, 2);
							}
						}
						...
						if (in_array($address['province'] . $address['city'], $a['aagentcitys'])) {
							...
							//$bonusmoney2 = round(($o['grprice'] * $a['bonus2']) / 100, 2);
							
							if($o['pcate']==1176){
								$bonusmoney2 = round(($o['grprice'] * $a['bonus2']) / 100, 2);
							}else{
								$bonusmoney2 = round(($o['grprice'] * $a['bonus5']) / 100, 2);
							}
						}
						...
						if (in_array($address['province'] . $address['city'] . $address['area'], $a['aagentareas'])) {
							...
							//$bonusmoney3 = round(($o['grprice'] * $a['bonus3']) / 100, 2);
							
							if($o['pcate']==1176){
								$bonusmoney3 = round(($o['grprice'] * $a['bonus3']) / 100, 2);
							}else{
								$bonusmoney3 = round(($o['grprice'] * $a['bonus6']) / 100, 2);
							}
						}
...
//v1.004 end
			...
			foreach ($aagents as &$a ) {
				...
				if ((floatval($set['paycharge']) <= 0) || ((floatval($set['paybegin']) <= $a['bonusmoney1'] + $a['bonusmoney2'] + $a['bonusmoney3'] + $a['bonusmoney4'] + $a['bonusmoney5'] + $a['bonusmoney6']) && ($a['bonusmoney'] <= floatval($set['payend'])))) {
					...
					$bonusmoney_send6 += round($a['bonusmoney6'], 2);
					...
					$bonusmoney_send6 += round($a['bonusmoney6'] - (($a['bonusmoney6'] * floatval($set['paycharge'])) / 100), 2);
					...
					$a['chargemoney6'] = round(($a['bonusmoney6'] * floatval($set['paycharge'])) / 100, 2);
				...
				$a['bonusmoney_send6'] = $bonusmoney_send6;
			...
			return array('orders' => $orders, 'aagents' => $aagents, 'ordermoney' => round($ordermoney, 2), 'ordercount' => count($orders), 'bonusmoney1' => round($bonusmoney1, 2), 'bonusmoney2' => round($bonusmoney2, 2), 'bonusmoney3' => round($bonusmoney3, 2), 'bonusmoney4' => round($bonusmoney4, 2), 'bonusmoney5' => round($bonusmoney5, 2), 'bonusmoney6' => round($bonusmoney6, 2), 'aagentcount1' => $aagentcount1, 'aagentcount2' => $aagentcount2, 'aagentcount3' => $aagentcount3, 'starttime' => $starttime, 'endtime' => $endtime, 'old' => false);
