        ...
        if( !empty($dataurl) && strpos($dataurl, "/pages/") === false ) 
        {
            ...
            else
            {
                if( $dataParams[0] == "groups" ) 
                {
                    ...
                    $goodsql = "SELECT id,title,subtitle,thumb,hascommission,nocommission,commission,commission1_rate,commission1_rate,marketprice,commission1_pay,maxprice,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM " . tablename("ewei_shop_goods") . " WHERE id in(" . $goodsid . ") AND uniacid =" . $_W["uniacid"] . " limit 0," . $goodsnum;
